...
class PlatformFFI {
  ...
  HbdeskImpl get ffiBind => _ffiBind;
  ...
  void _startListenEvent(HbdeskImpl hbdeskImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = hbdeskImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
