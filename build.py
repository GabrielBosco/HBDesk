...
hbb_name = 'hbdesk' + ('.exe' if windows else '')
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/hbdesk.exe')
    os.chdir('../..')
    if os.path.exists('./hbdesk_portable.exe'):
        os.replace('./target/release/hbdesk-portable-packer.exe',
                   './hbdesk_portable.exe')
    else:
        os.rename('./target/release/hbdesk-portable-packer.exe',
                  './hbdesk_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/hbdesk_portable.exe')
    os.rename('./hbdesk_portable.exe', f'./hbdesk-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/hbdesk-{version}-install.exe')
